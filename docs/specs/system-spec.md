...
|---|---|---|---|
| SUB-PR | Patient Records | Demographics, medical history, documents, consent, encrypted PHI, AI vision (wound assessment, patient ID verification, document OCR), dermatology CDS (EfficientNet-B4/MobileNetV3 skin lesion classification across 9 ISIC categories, pgvector similarity search against 50K reference embeddings, configurable threshold-based risk scoring, persistent lesion identity with longitudinal change detection via cosine distance), DermaCheck pipeline orchestration (parallel fan-out classification with graceful degradation, single-request lifecycle, per-stage timeout and audit), client-side capture-classify-review flow (branded "DermaCheck" entry point, CameraSessionManager camera integration, multipart image upload, synchronous result display, clinician action state machine: Save/Discard/Add Another) | All roles |
| SUB-CW | Clinical Workflow | Scheduling, encounters, status tracking, clinical notes, referrals, dermatology encounter integration (lesion assessments linked to encounters), DermaCheck encounter workflow (camera capture → image upload → results review → save/discard within encounter context), DermaCheck clinician action lifecycle (explicit Save/Discard/Add Another state machine with confirmation dialogs, multi-lesion capture sessions, degraded result handling) | Physicians, Nurses |
