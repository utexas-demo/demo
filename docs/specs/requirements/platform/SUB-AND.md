...
**Version:** 1.1
**Date:** 2026-02-24
**Platform:** Android (AND) — 21 requirements across 4 domains
...
| SUB-PR-0018-AND | SUB-PR-0018 | SYS-REQ-0014, SYS-REQ-0003 | Branded "DermaCheck" entry button on encounter detail screen, visible only when `DERM_CDS_ENABLED` feature flag is active. Camera capture via `CameraSessionManager` singleton with dermoscopy-specific `CameraProfile` (macro focus, high resolution, clinical white balance) applied during BINDING phase (SUB-PR-0012, PC-AND-01, PC-AND-03, RC-AND-01). Multipart/form-data upload via Retrofit with `patient_id`, `encounter_id`, and `anatomical_site` fields. Synchronous wait with `CircularProgressIndicator` during server processing. Results displayed atomically — full `DermaCheckResult` rendered after response completes, no progressive rendering. | `ui/dermatology/DermaCheckScreen.kt`, `ui/dermatology/DermaCheckViewModel.kt`, `data/api/LesionApi.kt` | TST-PR-0018-AND | Not Started |
...
| SUB-CW-0010-AND | SUB-CW-0010 | SYS-REQ-0014, SYS-REQ-0003 | DermaCheck clinician action screen with three Compose buttons: **Save** (persists assessment to encounter via API), **Discard** (shows `AlertDialog` confirmation, then calls `DELETE /api/lesions/{id}` for secure deletion), and **Capture Another** (saves current and navigates back to camera capture within the same encounter). Degradation chips ("Narrative unavailable", "Similar images unavailable") displayed when `degraded` fields are present but do not disable action buttons. Buttons disabled during pending API calls with loading indicators. | `ui/dermatology/DermaCheckActionsPanel.kt`, `ui/dermatology/DermaCheckViewModel.kt` | TST-CW-0010-AND | Not Started |
