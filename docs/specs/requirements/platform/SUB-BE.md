...
| SUB-CW-0010-BE | SUB-CW-0010 | Voice biomarker screening API: `POST /api/screening/analyze` accepts audio upload (PCM 16-bit 16kHz, minimum 20 seconds), extracts acoustic features via librosa, runs Kintsugi open-source depression and anxiety models, and returns screening result (depression_score, anxiety_score, confidence, categories). `POST /api/screening/analyze-features` accepts pre-extracted feature vectors for client-side feature extraction. `GET /api/screening/health` returns engine status. Must validate encounter status is `in_progress` before accepting a screening request — return 422 Unprocessable Entity if the encounter is not `in_progress` (DC-CW-04). Must enforce at most one screening result per (patient_id, encounter_id) pair using a database unique constraint — return 409 Conflict if a screening already exists for the encounter (RC-BE-13). Must verify patient consent before storing results. Must call `audit_service.log_action` with action `VOICE_SCREENING` and resource_type `screening`. Patient_id stored in a dedicated `subject_hash` field (SHA-256 hash, never cleartext in audit log); `resource_id` stores the screening record UUID (PC-BE-09). Raw audio must be discarded immediately after feature extraction — only numerical feature vectors and screening results are stored. | `routers/screening.py`, `integrations/kintsugi/engine.py`, `integrations/kintsugi/features.py`, `models/screening.py` | TST-CW-0010-BE | Not Started |
