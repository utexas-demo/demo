...
This document defines the governance procedures for evolving the PMS three-tier requirements decomposition (System → Domain → Platform) and provides a systematic conflict and race condition analysis across all 52 domain requirements and 104 platform requirements.
...
| 4 | **Create platform requirements** — for each "yes" decision, add requirements to the consolidated platform file `platform/SUB-{PLATFORM}.md` (under the relevant domain section). Use the format `SUB-{domain}-{NNNN}-{PLATFORM}`. Update the Platform Decomposition index table in the corresponding `domain/SUB-{domain}.md`. | `platform/SUB-{PLATFORM}.md`, `domain/SUB-*.md` |
...
| 3 | **Create subsystem requirements documents** — create `docs/specs/requirements/domain/SUB-{CODE}.md` for domain requirements and add a new domain section to each applicable consolidated platform file `docs/specs/requirements/platform/SUB-{PLATFORM}.md`, following the template structure of existing subsystem docs. | `domain/SUB-{CODE}.md` (new), `platform/SUB-{PLATFORM}.md` (updated) |
...
| 5 | **Decompose into platform requirements** — for each domain requirement, add platform-specific requirements to the relevant domain section in `platform/SUB-{PLATFORM}.md` for each applicable platform (BE, WEB, AND, AI). Update the Platform Decomposition index table in `domain/SUB-{CODE}.md`. | `platform/SUB-{PLATFORM}.md`, `domain/SUB-{CODE}.md` |
...
| 4 | **Decompose into platform requirements** — for each new domain requirement, determine which platforms (BE, WEB, AND, AI) require implementation and add platform requirements to the relevant domain section in the consolidated `platform/SUB-{PLATFORM}.md` file. Update the Platform Decomposition index table in the domain file. | `platform/SUB-{PLATFORM}.md`, `domain/SUB-*.md` |
...
### SUB-CW — Clinical Workflow (5 conflicts)
...
| DC-CW-04 | **Voice screening encounter status constraint** — SUB-CW-0010 states clinicians can initiate screening "during encounters" but does not restrict which encounter statuses permit screening. The encounter state machine (SUB-CW-0007) defines four states: scheduled, in_progress, completed, cancelled. Allowing screening on a `scheduled` encounter (patient not yet present) is clinically nonsensical. Allowing screening on a `completed` or `cancelled` encounter creates data linked to a finalized record, violating the encounter lifecycle's terminal-state invariant. | SUB-CW-0010 vs SUB-CW-0007 | Medium | Restrict voice biomarker screening to encounters in `in_progress` status only. SUB-CW-0010-BE must validate encounter status before accepting a screening request — return 422 Unprocessable Entity if the encounter is not `in_progress`. Update SUB-CW-0010 to specify: "screening may only be initiated for encounters in `in_progress` status." | **Resolved** — SUB-CW-0010 updated with `in_progress` status constraint. SUB-CW-0010-BE updated to validate encounter status and return 422 on non-`in_progress` encounters. |
| DC-CW-05 | **Mood trend alert vs clinical alert system overlap** — SUB-CW-0011 fires alerts when voice biomarker scores jump > 0.2 between consecutive encounters. SUB-CW-0005 fires clinical alerts that — per DC-CW-03 — block encounter completion until acknowledged. If mood trend alerts are treated as clinical alerts, they could unintentionally block encounter completion even though the score change was detected across encounters (not within the current encounter's notes). If they are not clinical alerts, the system has two parallel alert mechanisms with no defined relationship. | SUB-CW-0011 vs SUB-CW-0005 | Medium | Mood trend alerts are a distinct alert type ("informational") that does NOT block encounter completion. They are displayed as advisory notifications on the patient detail page and encounter screen but do not participate in the SUB-CW-0005 pre-transition gate. SUB-CW-0011 should specify: "mood trend alerts are informational and do not block encounter status transitions." The SUB-CW-0005 clinical alert system is reserved for safety-critical conditions (lab values, drug interactions). | **Resolved** — SUB-CW-0011 updated to specify mood trend alerts are informational and do not block encounter status transitions. Distinct from SUB-CW-0005 clinical alerts. |
...
**Total intra-domain conflicts: 19 (17 resolved, 2 open)**
...
| PC-BE-09 | **Audit catalog extension for voice screening** — SUB-CW-0010-BE introduces new audit action string `VOICE_SCREENING` and resource type `screening`. These must be added to the audit event catalog established by PC-BE-03 without conflicting with existing action strings. The `patient_id` hashing requirement in the screening audit log is a novel pattern — existing audit entries store `resource_id` in cleartext. If both patterns coexist, audit log consumers (SUB-RA-0003-BE) must handle mixed ID formats (cleartext vs hashed) in the same query interface. | SUB-CW-0010-BE vs SUB-PR-0005-BE, SUB-CW-0004-BE, SUB-MM-0004-BE, SUB-PM-0005-BE | Medium | Add `VOICE_SCREENING` action string and `screening` resource type to the audit event catalog. The `VOICE_SCREENING` prefix is unique and does not collide with existing action strings. For the patient_id hashing: store the `patient_id` hash in a dedicated `subject_hash` field (not `resource_id`) so the audit schema remains consistent. `resource_id` stores the screening record UUID. The audit query interface filters by `resource_type = screening` and never exposes the cleartext patient_id for screening events. | **Resolved** — SUB-CW-0010-BE updated to specify `VOICE_SCREENING` action, `screening` resource type, `subject_hash` field for SHA-256 hashed patient_id, and `resource_id` for screening record UUID. Audit event catalog extended. |
...
| PC-WEB-04 | **Microphone contention with telehealth encounters** — SUB-CW-0010-WEB requires browser microphone access via `MediaRecorder` API for voice biomarker recording. SUB-CW-0006 defines `telehealth` as an encounter type, which typically uses browser-based WebRTC audio/video calling that also claims the microphone. If a clinician initiates voice biomarker screening during a telehealth encounter, the browser may have the microphone already captured by the telehealth stream. Requesting a second `getUserMedia` stream may fail, produce silence, or interrupt the telehealth call — behavior varies by browser. | SUB-CW-0010-WEB vs SUB-CW-0006 | Medium | SUB-CW-0010-WEB must detect whether the microphone is already in use before requesting access. If a telehealth session is active, show a warning: "Microphone is in use by telehealth — pause or end the call before starting voice screening." Alternatively, reuse the existing telehealth audio stream (capture the last 20+ seconds from the active `MediaStream`). The simpler approach (warning + block) is recommended for v1.0. | **Resolved** — SUB-CW-0010-WEB updated to detect active microphone use, show telehealth warning, and block recording initiation when microphone is in use. |
...
**Total cross-platform conflicts: 17 (15 resolved, 2 open)**
...
| RC-BE-13 | **Concurrent screening for same patient-encounter** — Two clinicians (or the same clinician in two browser tabs) simultaneously submit voice screening audio for the same patient within the same encounter (SUB-CW-0010-BE). Both requests pass validation, both extract features, and both store screening results. The mood trend service (SUB-CW-0011-BE) now has two data points for the same encounter, making the "consecutive encounter" trend calculation ambiguous — the >0.2 jump detection compares consecutive screenings, but two screenings from the same encounter may produce a false-positive alert or obscure a genuine trend. | SUB-CW-0010-BE, SUB-CW-0011-BE | Medium | Enforce at most one screening result per (patient_id, encounter_id) pair using a database unique constraint. The second concurrent request receives 409 Conflict with message "screening already recorded for this encounter." If a clinician needs to redo a screening (e.g., poor audio quality), they must explicitly delete the prior result first. The mood trend service uses only the single screening per encounter, ensuring clean longitudinal data. | **Resolved** — SUB-CW-0010-BE updated with unique constraint on (patient_id, encounter_id) and 409 Conflict on duplicate. |

### Web Frontend (WEB) — 3 race conditions
...
| RC-WEB-03 | **Voice recording interrupted by page navigation** — The clinician starts a 20-second voice biomarker recording (SUB-CW-0010-WEB) then navigates away from the encounter page (sidebar link, browser back button, or encounter status change by another user). The `MediaRecorder` may continue capturing in the background (wasting resources and violating the "discard immediately" principle), or the recording may be silently lost. If the recording completes in the background and auto-submits, the user sees no result because they have left the page. If the recording is lost, the user must restart with no indication of what happened. | SUB-CW-0010-WEB | Medium | Implement a navigation guard (`beforeunload` event + Next.js `router.events`) during active recording. If recording is in progress, show a confirmation dialog: "Voice screening is in progress — leaving will discard the recording. Continue?" On confirmed navigation, explicitly call `MediaRecorder.stop()` and discard the audio buffer. The `VoiceBiomarkerScreen` component's cleanup function (React `useEffect` return) must stop the `MediaRecorder` and release the `MediaStream` to prevent background capture. | **Resolved** — SUB-CW-0010-WEB updated with navigation guard (`beforeunload` + `router.events`), confirmation dialog, `MediaRecorder.stop()` on departure, and `useEffect` cleanup for `MediaStream` release. |
...
**Total race conditions: 19 (19 resolved, 0 open)**

---

## 6. Summary

All 55 conflicts and race conditions have been resolved at the requirements level. The 9 Dermatology CDS conflicts (v1.5) from SUB-PR-0013–0016 were resolved in v1.6 (2026-02-21). The 6 voice biomarker screening conflicts (v1.7) from SUB-CW-0010–0011 were resolved in v1.8 (2026-03-06). The requirements documents (domain/SUB-CW.md, platform/SUB-BE.md, platform/SUB-WEB.md) have been updated to incorporate each resolution. Implementation of the updated requirements is tracked by the individual requirement statuses in each file.

| Category | Count | Critical | High | Medium | Low | Resolved |
|---|---|---|---|---|---|---|
| Intra-domain conflicts | 19 | 0 | 7 | 10 | 2 | **19/19** |
| Cross-platform conflicts | 17 | 0 | 4 | 10 | 3 | **17/17** |
| Race conditions | 19 | 2 | 7 | 9 | 1 | **19/19** |
| **Total** | **55** | **2** | **18** | **29** | **6** | **55/55** |
...
### Resolved Conflicts — Voice Biomarker Screening (v1.8, 2026-03-06)

The following 6 conflicts from SUB-CW-0010–0011 (SYS-REQ-0014) were identified in v1.7 and resolved in v1.8:

**Medium (resolved):**
1. **DC-CW-04** — Voice screening encounter status constraint. SUB-CW-0010 updated with `in_progress` status constraint. SUB-CW-0010-BE validates encounter status (422 on non-`in_progress`).
2. **DC-CW-05** — Mood trend alert vs clinical alert overlap. SUB-CW-0011 updated to specify mood trend alerts are informational and do not block encounter status transitions.
3. **PC-BE-09** — Audit catalog extension for voice screening. SUB-CW-0010-BE updated with `VOICE_SCREENING` action, `screening` resource type, `subject_hash` field for hashed patient_id.
4. **PC-WEB-04** — Microphone contention with telehealth encounters. SUB-CW-0010-WEB updated to detect active mic use, show telehealth warning, and block recording.
5. **RC-BE-13** — Concurrent screening for same patient-encounter. SUB-CW-0010-BE updated with unique constraint on (patient_id, encounter_id) and 409 on duplicate.
6. **RC-WEB-03** — Voice recording interrupted by page navigation. SUB-CW-0010-WEB updated with navigation guard, confirmation dialog, `MediaRecorder.stop()` cleanup, and `useEffect` `MediaStream` release.
...
| SYS-REQ-0014 | DC-CW-04, DC-CW-05, PC-BE-09, PC-WEB-04, RC-BE-13, RC-WEB-03 |
...
| SUB-CW-0010 | DC-CW-04, DC-CW-05, PC-BE-09, PC-WEB-04, RC-BE-13, RC-WEB-03 |
| SUB-CW-0011 | DC-CW-05, RC-BE-13 |
